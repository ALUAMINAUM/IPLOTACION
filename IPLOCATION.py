...
import sys, os
from core.IPLOCATIONLib import IPLOCATIONLib
...
def main():
    ...
    #init lib
    IPLOCATIRequest = IPLLCATIONLib(args.target, logger, args.noprint)
    
    print(banner)
    
    #retrieve information
    if not IPLOCATIRequest.GetInfo(args.uagent, args.tlist, 
                                     args.ulist, args.proxy, args.xlist,
                                     args.csv, args.xml, args.txt, args.g):
        logger.PrintError("Retrieving IP LOCATION information failed.")
        sys.exit(8)
